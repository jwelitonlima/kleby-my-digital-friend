      ...
      <Section>
        <div className="grid lg:grid-cols-2 gap-10 lg:gap-12 items-center">
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }}>
            <div className="aspect-[3/4] max-w-[280px] md:max-w-sm mx-auto lg:mx-0 bg-card rounded-2xl overflow-hidden flex items-center justify-center">
          ...
          <div>
            ...
            <div className="space-y-3 text-muted-foreground text-[15px] md:text-[15px] leading-relaxed mt-4">
      ...
      <Section className="border-t border-border/40">
        ...
        <div className="max-w-md mt-6 space-y-0">
          {formacoes.map((item: string, i: number) => (
            <div key={i} className="flex items-start gap-4 py-4 md:py-3.5 border-b border-border/30 last:border-0">
              <span className="text-xs font-bold text-primary/30 pt-0.5 select-none">{String(i + 1).padStart(2, "0")}</span>
              <span className="text-[15px] md:text-[14px] text-muted-foreground">{item}</span>
            </div>
          ))}
        </div>
      </Section>
      ...
      <Section className="border-t border-border/40">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 md:gap-6 mt-8">
          {pilares.map((item: any, i: number) => (
            <div key={i} className="bg-card rounded-2xl md:rounded-xl p-5 md:p-5">
              <h3 className="text-[16px] md:text-base font-semibold mb-1.5">{item.title}</h3>
              <p className="text-[14px] md:text-sm text-muted-foreground leading-relaxed">{item.desc}</p>
            </div>
          ))}
        </div>
      </Section>
