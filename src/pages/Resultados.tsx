...
const Resultados = () => {
  ...
  const depoimentos = parseJson(c, "resultados_depoimentos", defaultDepoimentos);
  const metricas = parseJson(c, "resultados_metricas", defaultMetricas);

  return (
    <>
      <Section>
        <SectionLabel>{c?.resultados_page_label ?? 'Resultados'}</SectionLabel>
        <SectionTitle>{c?.resultados_page_title ?? 'Dados reais'}</SectionTitle>
        <SectionSubtitle className="mb-8 md:mb-10">{c?.resultados_page_subtitle ?? 'Números de quem treina com constância.'}</SectionSubtitle>

        <div className="grid grid-cols-3 gap-3 md:gap-4 mb-12 md:mb-14">
          {metricas.map((m: any, i: number) => (
            <div key={i} className="bg-card rounded-2xl md:rounded-xl p-4 md:p-5">
              <p className="text-xl md:text-3xl font-bold">{m.valor}</p>
              <p className="text-[10px] md:text-[11px] font-semibold tracking-label uppercase text-muted-foreground mt-1">{m.label}</p>
            </div>
          ))}
        </div>

        <div className="mb-12 md:mb-14">
          <span className="text-[11px] font-semibold tracking-label uppercase text-muted-foreground block mb-5 md:mb-6">Transformações</span>
          <div className="grid grid-cols-3 gap-3">
            {[1, 2, 3].map((i) => (
              <div key={i} className="aspect-[3/4] bg-card rounded-2xl md:rounded-xl flex items-center justify-center">
      ...
      <Section className="border-t border-border/40">
        ...
        <div className="max-w-lg mt-6 md:mt-8 space-y-6 md:space-y-5">
          {depoimentos.map((d: any, i: number) => (
            <motion.div key={i} initial={{ opacity: 0, y: 6 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: i * 0.05 }}>
              <p className="text-[15px] leading-relaxed text-foreground/90 mb-1.5 md:mb-1">"{d.texto}"</p>
